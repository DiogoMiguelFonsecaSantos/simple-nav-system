 ...
 */
 ...
 esp_err_t SPI_PROTOCOLInit(void) {
    ...
    vTaskDelay(pdMS_TO_TICKS(10)); // Wait for Supply VCI and Wait 10ms for epaper (Power-On)
    ...
    ret = spi_bus_add_device(MY_SPI_HOST, &dev_config_epaper, &spi_handle_epaper);
    return ret;
 }
